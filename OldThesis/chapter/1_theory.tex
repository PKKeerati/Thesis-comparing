...
The resulting forces acting on atoms in the structure are then obtained via the Hellman-Feynman theorm from these calculations.
...
In the next subsection, these force constants will be used in phonon calculations.
...
I have discussed the fundamental relations as used in \textsc{phonopy} and \textsc{phono3py}, which form the basis for understanding the functionalities and computational procedures which underly these widely-used tools for phonon and thermal transport calculations.
...
The accuracy of a DFT calculation is also strongly influenced by the selection of the exchange-correlation (XC) functional, which approximates the many-body effects of electron exchange and correlation.
Common categories of XC functionals are
\begin{itemize}
    \item[1] Local Density Approximation (LDA): Relies solely on the local electron density, parametrised using quantum Monte Carlo results for the homogeneous electron gas (HEG), and is most accurate for systems with slowly varying electron density, such as metals.
    \item[2] Generalised Gradient Approximation (GGA): Improves upon LDA by incorporating the gradient of the electron density, allowing for greater accuracy in systems with sharply varying electron densities, such as insulates and semiconductor. Popular examples include the Perdew-Burke-Ernzerhof (PBE) functional.
    \item[3] Hybrid Functionals: Combine DFT correlation with exact Hartree-Fock exchange, providing a more accurate treatment by justifying electron interactions. Examples include B3LYP, widely used in molecular chemistry, and HSE06, commonly applied in solid-state calculations. 
\end{itemize}
The choice of functional depends on the type of system being studied and the level of accuracy required for the calculation, and available computational resources.
...
A random vector (descriptor), $\mathbf{x}$, can be indexed by time, space, or other inputs.
A GP is fully characterised by its mean function and covariance function.
These functions are defined for a real function $f(\textbf{x})$ as:
\begin{equation}
    \label{equ:2.28}
    \begin{split}
    ...
    \end{split}
\end{equation}
where $m(\textbf{x})$ represents the expected value (mean) of the function at descriptor $\mathbf{x}$, and $k(\textbf{x},\;\textbf{x}^{*})$ defines the covariance between the function evaluated at $\mathbf{x}$ and $\mathbf{x}^{*}$.
The Gaussian process, fully defined by its mean function and covariance function, can be interpreted as a prior distribution over the function, 
mathematically, this is
\begin{equation}
    \label{equ:2.29}
    f(\mathbf{x}) \sim \mathcal{GP} \left( m(\mathbf{x}), k(\mathbf{x},\; \mathbf{x}^{*}) \right),
\end{equation}
where $m(\textbf{x})$ and $k(\textbf{x},\;\textbf{x}^{*})$ are defined in equation ~\eqref{equ:2.28}, describing the relationship between function values at different points in the input space.
This means that the real function $f(\textbf{x})$ is distributed over an average function $m(\textbf{x})$ and a covariance function $k(\textbf{x},\:\textbf{x}^{*})$.
...
This noise arises from electronic structure calculations and can be expressed as \[\tilde{E}(\mathbf{x})=E(\mathbf{x})\pm \epsilon,\] where the uncertainty $\epsilon$ can be presumed to be normal distributed with a mean of zero and a variance of $\sigma^{2}$.
This is defined as:
\begin{equation}
    \label{equ:2.34}
        \epsilon\\
...
\end{equation}
In physics, Gaussian processes (GPs) have been used extensively.
As previously mentioned, GPs can be used for modelling the electronic potential energy surfaces (PESs) of \(N\)-atomic systems with the help of previous information.
Usually, electronic structure computations like density functional theory (DFT), which yield potential energies and atomic forces at specific input structures, are the source of these data.

One structure and its associated potential energy can be represented for GP modelling as a scalar target (\(E_{i}\)) and a \(3N\)-dimensional Cartesian descriptor (\(\mathbf{x}_{i}\)), respectively.
A total of \(n\) structures can be arranged into a \(3N \times n\) descriptor matrix (\(\mathbf{X}\)) and a \(n\)-dimensional target vector (\(\mathbf{E}\)) in order to create a \textit{training} or \textit{test} dataset that is compatible with GP models.
...
This \textbf{Hessian} matrix includes the correlation of the prior forces at \(\mathbf{x}\), expressed as the $3N\times 3N$ second-rank tensor,
\begin{equation}
    \label{equ:2.40}
    \nabla_{\mathbf{x}}\nabla_{\mathbf{x}} k
    =
...
\begin{equation}
    \label{equ:2.41}
        ...
        \begin{bmatrix}
         ...
         \end{bmatrix}\right),
\end{equation}  
where $\nabla_{\mathbf{x}} E = -\left[F_{x,1}\;F_{y,1}\;F_{z,1}\cdots F_{x,N}\;F_{y,N}\;F_{z,N} \right]^{\mathbf{T}}$ are the prior forces at $\mathbf{x}$ and $k|_{\mathbf{x},\:\mathbf{x}^{*}} ~\equiv k(\mathbf{x},\:\mathbf{x}^{*})$.
...
$\Sigma_{f}$ is the $3N \times 3N$ variance matrix associated with the observation noise of the prior forces, defined as:
\begin{equation}
    \label{equ:2.42}
...
Examples of local atomistic descriptors are \textbf{Atom-Centred Symmetric Functions: ACSF} \cite{Behler2007,Behler2011}; \textbf{Smooth Overlap Atomic Positions: SOAP} \cite{Bartk2013,Bartk2015,Deringer2021}; \textbf{(Multi) Atomic Cluster Expansion: (M)ACE} \cite{Drautz2019, Musil2021, Dusson2022, Witt2023}; and the \textbf{Message Passing} version of \textbf{ACE}, \textbf{MACE} \cite{https://doi.org/10.48550/arxiv.2206.07697, Batatia2025}.
...
\begin{equation}
    \label{equ:2.46}
     E_{\text{tot}} = \sum_{\alpha}\sum_{j\:\in\:\alpha}\epsilon_{j}^{\alpha} + \mathcal{V}_{\text{long range}}.
\end{equation}
...
Pair distances for two-body interactions, bond angles for three-body interactions, and dihedral angles for four-body interactions are examples of these descriptors.
...
This prediction is performed using a prior dataset obtained from electronic structure calculations, enabling accurate and data-efficient modelling of interatomic interactions.
...
\end{equation}
This kernel function must be reconstructed to a covariance matrix for predicting the \textit{local} atomic energy $\epsilon^{\star}$ of a descriptor $\textbf{d}^{\star}$ from a prior dataset $\textbf{E}$. 
If a prior dataset $\textbf{E}= \left[\epsilon^{1}\:\epsilon^{2}\cdots\epsilon^{n} \right]^{\text{T}}$ contains $n$ \textit{local} atomic energies corresponding to individual $n$ atomic descriptors in the set of descriptors $\textbf{D}= \left[\textbf{d}_{1}\:\textbf{d}_{2}\cdots\textbf{d}_{n} \right]^{\text{T}}$, the $n\times n$ covariance matrix over the prior dataset is defined as $K(\textbf{D}) \propto \langle \textbf{E}\textbf{E}^{\text{T}} \rangle$. 
...
\begin{equation}
    \label{equ:2.52}
      \textbf{d}_{i}(\textbf{r})~\equiv \rho_{i}(\textbf{r}) = \sum^{\text{neigh.}}_{j} \text{exp}\left(-\frac{|\textbf{r}-\textbf{r}_{ij}|^{2}}{2\sigma^{2}}\right).
\end{equation}
This descriptor includes Gaussian peaks specifying the spatial distribution of neighboring atoms $j$.
...
We can employ this descriptor to rewrite Equation (\ref{equ:2.49}) in terms of atomic energy $\epsilon^{i}$ as a functional of the atomic density $\rho_{i}$,
\begin{equation}
    \label{equ:2.53}
...
\end{equation}
By integrating all the associated density functions, this formulation guarantees that the kernel function captures the similarity between atomic environments. However, this kernel function is not invariant under 3D rotations due to the chosen descriptor $\rho_{i}$. 
...
$R_{nl}(r)$ is a radial basis function depending on the magnitude of vector $\textbf{r}$ and $Y_{lm}$ is a spherical harmonic basis function depending on the direction of vector $\textbf{r}$.
...
Instead of calculating the tensor product of the basis functions before sums over $j$, we perform a scalar multiplication of $\sum^{J-1}_{j}\phi_{k}(\mathbf{r}_{ij})$ leading to an $N$ linear scale for evaluating an $N$-interaction order term, compared to an $N!$ scaling in the canonical cluster expansion. Each of the sums can be defined as in \textbf{Atomic neighbour density basis}   
\begin{equation}
\label{equ:2.68}
\begin{split}
...
We can incorporate the single particle basis function, Equation~\eqref{equ:2.55} into the above transformation.
...
Multi-element systems include additional atomic number z-dependence, resulting in the configuration space greater in dimension and can dramatically decrease the density projection's efficiency.  The practical implications of this efficiency reduction and solutions of minimising it, such as through compositional structures or sparsification, have not yet been thoroughly investigated.
